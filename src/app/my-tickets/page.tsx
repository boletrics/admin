...
export default function MyTicketsPage() {
	...
	const upcomingOrders = ordersWithEvents.filter((order) => {
		if (!order.event || !order.event.dates || order.event.dates.length === 0)
			return false;
		return new Date(order.event.dates[0].date) >= now;
	});
								...
								<div className="flex-1">
									...
									<div className="space-y-1 text-sm text-muted-foreground">
										<div className="flex items-center gap-2">
											...
											<span>
												{order.event.dates && order.event.dates.length > 0
													? `${formatDate(order.event.dates[0].date)}${order.event.dates[0].times && order.event.dates[0].times.length > 0 ? ` • ${order.event.dates[0].times[0]}` : ""}`
													: "Date TBD"}
											</span>
										</div>
										<div className="flex items-center gap-2">

...
import { Ticket, DollarSign, Calendar, Users, ArrowUpRight, ArrowDownRight } from "lucide-react"
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div className="flex items-center gap-2 mt-1">
          {trend !== undefined && (
            <Badge variant={isPositive ? "default" : "destructive"} className="gap-1 px-1.5 py-0.5 text-xs">
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {formatNumber(event.ticketsSold)} / {formatNumber(event.capacity)} boletos
                  </p>
                </div>
                <div className="text-right ml-4">
                  ...
                  <Badge
                    variant={
                      event.soldPercentage >= 90 ? "default" : event.soldPercentage >= 70 ? "secondary" : "outline"
                    }
                    className="mt-1"
                  >
...
export function OrgDashboardOverview() {
  ...
  const eventPerformance = useMemo<OrgEventPerformance[]>(() => {
    if (!currentOrg) return []
    return mockEventPerformance[currentOrg.id] || mockEventPerformance["org-1"] || []
  }, [currentOrg])
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Welcome Header */}
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Bienvenido, {currentOrg?.name}</h1>
      ...
      {/* Event Performance & Quick Actions */}

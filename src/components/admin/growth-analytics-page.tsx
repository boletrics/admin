...
import { Users, Building2, ArrowUpRight, ArrowDownRight, Target, UserPlus } from "lucide-react"
...
export function GrowthAnalyticsPage() {
  ...
  const userGrowthRate = (
    ((mockGrowthMetrics.userGrowth[6].users - mockGrowthMetrics.userGrowth[0].users) /
      mockGrowthMetrics.userGrowth[0].users) *
    100
  ).toFixed(1)
  const tenantGrowthRate = (
    ((mockGrowthMetrics.tenantGrowth[6].tenants - mockGrowthMetrics.tenantGrowth[0].tenants) /
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">Track platform growth and user acquisition</p>
      </div>
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={mockGrowthMetrics.userGrowth}>
                ...
                <YAxis yAxisId="left" tickFormatter={(v) => `${v / 1000}k`} className="text-xs" />
                ...
                <Area
                  ...
                  strokeWidth={2}
                />
                <Bar yAxisId="right" dataKey="newUsers" fill="var(--color-newUsers)" radius={[4, 4, 0, 0]} />
              </ComposedChart>
            </ResponsiveContainer>
          </ChartContainer>
        </CardContent>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>New tenants and churned tenants comparison</CardDescription>
          </CardHeader>
          <CardContent>
            <ChartContainer
              config={{
                new: { label: "New Tenants", color: "hsl(142.1, 76.2%, 36.3%)" },
        ...
        <Card>
          ...
          <CardContent>
            <ChartContainer
              config={{
                rate: { label: "Conversion Rate", color: "hsl(var(--chart-1))" },
              }}
              className="h-[300px]"
            >
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={mockGrowthMetrics.conversionRate}>
                  <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
                  <XAxis dataKey="month" className="text-xs" />
                  <YAxis tickFormatter={(v) => `${v}%`} className="text-xs" domain={[0, 6]} />
                  <ChartTooltip content={<ChartTooltipContent />} />
                  <Line
                    type="monotone"
                    dataKey="rate"
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Key growth indicators for the past 6 months</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-6 md:grid-cols-3">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">Total tenants added (net of churn)</p>
            </div>
            <div className="space-y-2">
              <h4 className="font-medium">Net User Growth</h4>
              <p className="text-3xl font-bold text-emerald-600">+7,332</p>
              <p className="text-sm text-muted-foreground">Total users added across all tenants</p>
            </div>
            <div className="space-y-2">
              <h4 className="font-medium">Churn Rate</h4>
              <p className="text-3xl font-bold text-amber-600">0.8%</p>
              <p className="text-sm text-muted-foreground">Monthly tenant churn rate</p>
            </div>
          </div>
        </CardContent>
      </Card>

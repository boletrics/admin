...
export type OrganizationRole = "owner" | "admin" | "manager" | "staff" | "readonly"
  ...
  persist(
    (set, get) => ({
        ...
        set((state) => ({
          ...
          currentOrg:
            state.currentOrg?.id === id
              ? { ...state.currentOrg, ...updates, updatedAt: new Date().toISOString() }
              : state.currentOrg,
        })),
      ...
      getCurrentMember: () => {
        ...
        return members.find((m) => m.organizationId === currentOrg.id && m.userId === MOCK_CURRENT_USER_ID) || null
      },
    }),
    {
      name: "org-storage",
